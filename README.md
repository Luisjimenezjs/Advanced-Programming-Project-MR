...
I click *Generate Standard Report*. The platform generates a comprehensive, standardized report where every research objective is paired with its corresponding charts and a deep, AI-written analysis. This complete document is visualized on screen and ready to download as a polished deliverable.
...
*   **out:** Dashboards structured by research objective; AI-generated standard reports containing charts and insights; machine learning segmentation profiles (K-means/K-prototypes) with AI-written narratives.
*   **on screen:** A Setup module for questionnaire mapping; a main view divided by Sections/Objectives; an Export module for the full report; an Advanced Analytics module for variable selection and cluster visualization.
...
*   **First useful version:**
    ...
    *   Standardized Reporting: The system pairs visual charts with AI-driven insights for each objective, allowing for a complete, downloadable report.
...
*   Given a click on "Generate Report", the system compiles all charts and AI analyses into a single, cohesive document that perfectly mirrors the established research objectives.
...
*   **API Token Optimization:** The system must strictly enforce a "one-shot" generation and caching mechanism. If the application state fails to cache the generated report and makes redundant API calls to Gemini every time a user views or downloads it, it will cause severe token bloat and unmanageable costs.
